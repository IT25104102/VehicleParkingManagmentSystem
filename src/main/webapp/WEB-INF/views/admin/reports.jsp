...
<head>
    ...
    <title>MyParking | System Reports</title>
    ...
    <style>
        ...
        .tile .t-lbl { font-size: 0.65rem; color: var(--text-dim); margin-top: 5px; text-transform: uppercase; letter-spacing: 0.06em; font-weight: 700; }
        .price-row { margin-bottom: 24px; display: flex; align-items: center; gap: 12px; }
        .price-row span { font-size: 0.85rem; color: var(--text-dim); }
        ...
        .section-header .count { font-size: 0.75rem; color: var(--text-dim); }
        .tab-nav { display: flex; gap: 4px; padding: 12px 20px; border-bottom: 1px solid rgba(255,255,255,0.08); flex-wrap: wrap; }
        .tab-btn { padding: 6px 14px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.08); background: transparent; color: var(--text-dim); font-family: 'Montserrat',sans-serif; font-size: 0.75rem; font-weight: 700; cursor: pointer; transition: 0.3s; }
        ...
        thead th { padding: 10px 18px; font-size: 0.7rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--text-dim); border-bottom: 1px solid rgba(255,255,255,0.08); text-align: left; }
        tbody td { padding: 10px 18px; font-size: 0.85rem; border-bottom: 1px solid rgba(255,255,255,0.05); }
        tbody tr:last-child td { border-bottom: none; }
        tbody tr:hover { background: rgba(255,255,255,0.03); }
        .empty-state { padding: 28px; text-align: center; color: var(--text-dim); font-size: 0.85rem; }
...
<div class="page-content">
    ...
    <div class="price-row">
        ...
        <a href="<%= request.getContextPath() %>/admin/price">Change &rarr;</a>
    </div>
    ...
    <div class="summary-grid">
        ...
        <div class="tile">
            <div class="t-val c-teal">Rs.<%= String.format("%.0f", report.getTotalIncome()) %></div>
    ...
    <div class="section-card">
        ...
        <div id="tab-users" class="tab-content show">
            <% if (report.getUserLines().isEmpty()) { %>
                <div class="empty-state"><i class="fas fa-user-slash"></i>No users data yet</div>
            <% } else { %>
                <table>
        ...
        <div id="tab-vehicles" class="tab-content">
            <% if (report.getVehicleLines().isEmpty()) { %>
                <div class="empty-state"><i class="fas fa-car-side"></i>No vehicles data yet</div>
            <% } else { %>
                <table>
        ...
        <div id="tab-slots" class="tab-content">
            <% if (report.getSlotLines().isEmpty()) { %>
                <div class="empty-state"><i class="fas fa-parking"></i>No slots data yet</div>
            <% } else { %>
                <table>
                    <thead><tr><th>#</th><th>Slot Data</th><th>Status</th></tr></thead>
                    <tbody>
        ...
        <div id="tab-tickets" class="tab-content">
            <% if (report.getTicketLines().isEmpty()) { %>
                <div class="empty-state"><i class="fas fa-ticket-alt"></i>No tickets data yet</div>
            <% } else { %>
                <table>
        ...
        <div id="tab-payments" class="tab-content">
            <% if (report.getPaymentLines().isEmpty()) { %>
                <div class="empty-state"><i class="fas fa-money-check-alt"></i>No payments data yet</div>
            <% } else { %>
                <table>
                ...
                <div class="total-row">
                    <strong class="c-green">Total: Rs.<%= String.format("%.2f", report.getTotalIncome()) %></strong>
                </div>
            <% } %>
        </div>
        ...
        <div id="tab-logs" class="tab-content">
            <% if (report.getLogLines().isEmpty()) { %>
                <div class="empty-state"><i class="fas fa-file-alt"></i>No logs yet</div>
            <% } else { %>
                <table>
                    <thead>
                        <tr>
